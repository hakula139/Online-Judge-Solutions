...
class ExpressionTree {
 ...
 protected:
  ...
  void InitOutput();
  void InitOutput(Node<char>* node, size_t pos_x, size_t pos_y,
                  size_t space_width);
...
void ExpressionTree::InitOutput(Node<char>* node, size_t pos_x, size_t pos_y,
                                size_t space_width) {
  output_.at(pos_y).at(pos_x) = node->val_;
  if (node->left_) {
    output_.at(pos_y + 1).at(pos_x - 1) = '/';
    InitOutput(node->left_, pos_x - space_width, pos_y + 2, space_width >> 1);
  }
  if (node->right_) {
    output_.at(pos_y + 1).at(pos_x + 1) = '\\';
    InitOutput(node->right_, pos_x + space_width, pos_y + 2, space_width >> 1);
  }
}
